 ...
 * especially with AN_CC_LIKELY.  Make sure our helpers are really
 * inlined.
 */
#define INLINE AN_CC_INLINE static
...
an_itoa(char *out, uint32_t x)
{
	 ...
	 */
	if (AN_CC_LIKELY(x < PO2)) {
		return itoa_hundred(out, x);
	}

	if (AN_CC_LIKELY(x < PO4)) {
